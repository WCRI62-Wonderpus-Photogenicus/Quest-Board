  ...
  return (
    <div className='task-modal'>
      <div className='modal-header'>
        <button onClick={() => dispatch(deleteTaskActionCreator(props.id))}>
          Delete
        </button>
        <button onClick={() => dispatch(toggleTaskModalActionCreator(false))}>
          CLOSE
        </button>
      </div>
      ...
      ></input>
      <button
        onClick={() => dispatch(editTaskDescActionCreator(props.id, descText))}
      >
        SAVE
      </button>
    </div>
